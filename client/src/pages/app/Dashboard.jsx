...
const GlowCard = ({ children, className = '', glow = 'purple', noBg = false }) => {
  ...
  return (
    <div className={`relative rounded-2xl border ${g.border} ${g.shadow} ${noBg ? '' : 'bg-[#07060f]/90'} backdrop-blur-xl overflow-hidden ${className}`}>
      {/* Top shimmer line */}
...
const CardHeader = ({ icon: Icon, iconBg, label, title, right }) => (
  <div className="flex items-center justify-between mb-4">
    <div className="flex items-center gap-2.5">
      <div className={`w-8 h-8 rounded-xl ${iconBg} flex items-center justify-center shadow-inner`}>
        <Icon size={14} />
      </div>
      <div>
        <p className="text-[8px] font-orbitron font-black tracking-[0.18em] text-muted/60 uppercase">{label}</p>
...
const PriorityBadge = ({ p }) => {
  ...
  return (
    <span className={`text-[7px] font-orbitron font-extrabold px-1.5 py-0.5 rounded border tracking-wider ${styles[p] || styles.medium}`}>
      {p?.toUpperCase()}
    </span>
  );
};
    ...
    <div className="relative min-h-screen space-y-6 pb-10">
      ...
      <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex-1">
          ...
          <h1 className="mt-1 font-orbitron text-2xl font-black lg:text-3xl tracking-widest">
            HARVOX <span className="gradient-text-animated">Command Center</span>
          </h1>
          <p className="text-[10px] text-gray-600 mt-1 font-mono tracking-widest">
        ...
        {/* Live metric strip */}
        <div className="flex gap-2.5 flex-wrap lg:flex-nowrap lg:flex-col lg:gap-2 lg:min-w-[180px] shrink-0">
          {metricCards.map(({ label, value, color, glow, icon: Icon, suffix }) => (
            <motion.div
              key={label}
              animate={{ opacity: [0.85, 1, 0.85] }}
              transition={{ duration: 3, repeat: Infinity }}
              className="flex items-center gap-3 bg-[#07060f]/80 border border-white/8 rounded-2xl px-3 py-2.5 backdrop-blur-sm relative overflow-hidden"
            ...
            >
              ...
              <div>
                ...
                <p className="text-[8px] text-gray-600 uppercase tracking-wider mt-0.5 font-mono">{label}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {QUICK_COMMANDS.map(({ label, icon: Icon, path, prompt, neon, glow }) => (
            <motion.button
              ...
              className="relative bg-[#07060f]/90 border border-white/8 rounded-2xl p-4 text-left group overflow-hidden transition-all duration-300"
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      className="flex-1 bg-white/5 border border-white/10 rounded-xl px-2 py-1.5 text-[10px] text-gray-400 outline-none focus:border-neon-purple/30 transition-all"
                    >
              ...
              {loading ? (
                ...
                tasks.map(task => (
                  <motion.div
                    ...
                    className={`group flex items-start gap-2.5 p-2.5 rounded-xl border transition-all duration-300 ${
                  ...
                  >
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1 hover:bg-rose-500/10 rounded-lg text-gray-700 hover:text-rose-400 transition-all shrink-0"
                    >
                      <Trash2 size={10} />
                    </button>
                  </motion.div>
        ...
        <GlowCard className="lg:col-span-2" glow="cyan">
          <div className="p-5">
            <CardHeader
              ...
              right={
                <span className="text-[9px] font-mono font-black text-neon-blue/80 bg-neon-blue/8 border border-neon-blue/20 px-2.5 py-1 rounded-full shadow-[0_0_10px_rgba(0,240,255,0.15)]">
                  {totalStudy}h TOTAL
                </span>
              }
            />
            ...
            <div className="flex gap-2 mb-4">
              <select
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 text-[10px] text-gray-300 outline-none focus:border-neon-blue/40 focus:shadow-[0_0_8px_rgba(0,240,255,0.15)] transition-all"
              >
              ...
              <input
                ...
                placeholder="hrs"
                className="w-16 bg-white/5 border border-white/10 rounded-xl px-2 py-1.5 text-[10px] text-gray-300 outline-none text-center focus:border-neon-blue/40 transition-all"
              />
              <button
                onClick={handleLogStudy}
                disabled={logStudy || !studyHours}
                className="px-3.5 py-1.5 bg-neon-blue hover:bg-neon-blue/80 rounded-xl text-[10px] font-black text-black transition-all disabled:opacity-50 flex items-center gap-1 shadow-[0_0_12px_rgba(0,240,255,0.3)]"
              >
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <CardHeader
              ...
              right={
                <Link
                  ...
                  className="text-[9px] font-mono text-emerald-400/70 flex items-center gap-1 hover:text-emerald-400 transition-colors"
                >
                  ...
                  return (
                    <div key={proj._id || i} className="flex items-center gap-3 p-3 bg-white/3 border border-white/5 rounded-2xl hover:border-emerald-500/20 transition-all duration-300 group">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[7px] text-gray-600 font-mono mt-1 tracking-wider">{proj.framework || 'MERN'} · {new Date(proj.updatedAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                  );
                })
        ...
        <GlowCard glow="gold">
          <div className="p-5">
            ...
            {/* Terminal-style log */}
            <div
              className="bg-black/60 border border-white/8 rounded-2xl p-3 max-h-72 overflow-y-auto font-mono text-[9px] space-y-1.5 shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] relative"
        ...
        <GlowCard glow="pink">
          <div className="p-5">
            <CardHeader
              ...
              right={
                <Link to="/app/chat" className="text-[9px] font-mono font-bold text-neon-pink/70 flex items-center gap-1 hover:text-neon-pink transition-colors">
