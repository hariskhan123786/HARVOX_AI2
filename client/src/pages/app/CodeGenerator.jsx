...
import { Download, Play, Copy, Check, Code2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
            ...
            <GlassCard hover={false} className="p-0 overflow-hidden">
              ...
              <div className="flex items-center justify-between border-b border-white/10 px-4 py-2 bg-secondary/30">
                ...
                <div className="flex items-center gap-2">
                  {['Python', 'JavaScript', 'C++'].includes(language) && (
                    <button
                      onClick={() => setShowExecutor(!showExecutor)}
                      disabled={!editedCode || loading}
                      className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs bg-neon-purple/10 text-neon-purple hover:bg-neon-purple/20 transition-colors disabled:opacity-40"
                    >
                      <Play size={12} /> Run
                    </button>
                  )}
                  <button
                    ...
                    disabled={!editedCode}
                    className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs bg-white/5 text-muted hover:text-white transition-colors disabled:opacity-40"
                  >
              ...
              {/* Code Executor */}
              {showExecutor && (
                <div className="border-t border-white/10 bg-[#070B14]">
                  <CodeExecutor
                    initialCode={editedCode}
                    initialLanguage={language === 'Node.js' ? 'javascript' : language.toLowerCase()}
                  />
                </div>
              )}
            </GlassCard>
      ...
      <ExportModal
        ...
        data={{ code: editedCode, language }}
        type="code"
        metadata={{
          title: `${language} Code`,
          description: prompt,
          filename: `harvox-${language.toLowerCase()}-${Date.now()}`
        }}
      />
    </div>
  );
